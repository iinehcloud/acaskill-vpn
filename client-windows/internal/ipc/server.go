...
enriched[i] = ifaceWithState{NetworkInterface: iface, IsActive: activeMap[iface.FriendlyName]}
}
...
case MsgSetLicense:
var p SetLicensePayload
if err := json.Unmarshal(req.Payload, &p); err != nil {
return errResp(req.ID, "invalid payload")
}
if p.LicenseKey == "" {
return errResp(req.ID, "license key required")
}
// Validate with server API
if err := s.cfg.ValidateAndRegister(p.LicenseKey, p.DeviceName); err != nil {
return errResp(req.ID, err.Error())
}
return okResp(req.ID)
